...
// HomeV12: Calculator positioned after editorial statement, before service panels
...
export default function HomeV12() {
  ...
  return (
    <div className="min-h-screen bg-charcoal">
      ...
      {/* 6. Showcase video: hidden until video is ready */}
      <section className="hidden py-20 lg:py-32 border-t border-gold/10">
        <div ref={videoRef} className="max-w-[1400px] mx-auto px-8 lg:px-20" style={{ opacity: videoInView ? 1 : 0, transform: videoInView ? 'translateY(0)' : 'translateY(28px)', transition: 'opacity 0.9s ease-out, transform 0.9s ease-out' }}>
          <div className="flex items-end justify-between mb-10">
            <h2 className="font-display text-[clamp(2.2rem,5vw,6rem)] font-light italic text-cloud-white leading-none">See for yourself.</h2>
            <Link href="/en/gallery" className="hidden sm:inline-block text-sm tracking-[0.1em] uppercase text-gold/55 hover:text-gold transition-colors duration-300 border-b border-gold/20 hover:border-gold/55 pb-0.5 mb-1">Full gallery →</Link>
          </div>
          <div className="h-px bg-gold/18 mb-10 origin-left" style={{ transform: videoInView ? 'scaleX(1)' : 'scaleX(0)', transition: 'transform 1s cubic-bezier(0.77, 0, 0.175, 1) 0.15s' }} />
          <div className="relative aspect-video overflow-hidden group cursor-pointer border border-gold/12 hover:border-gold/30 transition-colors duration-500 bg-charcoal/50">
            ...
            <p className="absolute bottom-6 left-6 text-cloud-white/40 text-xs tracking-[0.18em] uppercase">Property tour · Sofia</p>
          </div>
        </div>
      </section>
